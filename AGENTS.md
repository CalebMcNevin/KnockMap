...
- Help tour: 8-step walkthrough (`TOUR` array; card is always centered, the spot is the cutout in the dark overlay via a 9999px box-shadow) auto-runs once after the first CSV upload unless `tourDone` is set in `dkm-prefs`; the compass button `#tourBtn` in the header replays it (the last step's cutout lands on it). Veil/spot/card are `popover="manual"` because showing an auto popover closes every other open popover that is not its DOM ancestor (the trio must coexist; Esc and veil clicks end the tour in JS). During the tour `tourStep` also flips `#settingsPanel` itself to `popover="manual"` (otherwise a click on the card light-dismisses the auto panel and desyncs the `tourPanel` flag; `tourEnd` restores `"auto"`). Only the Settings step opens `#settingsPanel` (animated via `@starting-style`); it works because the panel is shown before the tour popovers, so the top layer stacks veil/spot/card above it, and `tourStep` re-measures 220ms later once the entrance animation finishes. The view-toggle step carries a phased `t2` chain: `t2: [["#viewPermBtn", 1000], ["#permitLegend", 2000]]` first moves the spotlight onto the toggle's Permits half and `permitClickDemo` visibly presses it (`.pressed`, green + scale) with that press doing the real `setView(true)` flip (`tourFlipped`, restored by `tourEnd` only if the tour itself flipped it), then the spotlight moves to the legend's Knocks / Door Hangers picker and `permitActDemo` expands a replica dropdown `#tourActDD` under it listing both options with the current one highlighted (native `select.showPicker()` needs live user activation and cannot be closed programmatically; the replica is removed on any step change and by `tourEnd`). The rule-tracker step (`t: "#permitsBtn"`, `t2: [["#permitsBtn", 1000], ["#permitDialog", 2000]]`) does the same with `editPermitsClickDemo` pressing Edit Permits to really open the form while the highlight holds on the button for another second before the `#permitDialog` phase moves the spotlight onto the form (`tourPlacePermitDlg` puts it beside the centered card with a 50px gap; `margin: 0` inline, else the UA dialog centering re-ignores `left`; `tourDlg` claimed so `tourEnd` closes it and any step change closes it, re-entering step 7 replays the press) (out-of-order throttled timers are skipped by the `tourT2 !== k` guard)
