...
- First view per report: on load the map centers on the median centroid of the report's codes at `maxZoom - 7` (bulk-centered so one outlier zip can't drag the view; `fitBounds` only if no code has a centroid); a restored session view (`mv` in dkm-prefs) wins instead
